...
public class MonstersNMSInvokerV1_19_1 extends MonstersNMSInvoker {

    @Override
    public void dealExplosionDamage(Player player, int amount) {
        ServerPlayer serverPlayer = ((CraftPlayer) player).getHandle();
        serverPlayer.hurt(DamageSource.explosion((Explosion) null), amount);
    }
    ...
    @Override
    public Goal<Villager> getVillagerAfraidGoal(LivingEntity villager, Predicate<Player> hasAbility) {
    ...
    public void launchArrow(Entity projectile, Entity entity, float roll, float force, float divergence) {
        ((Projectile) ((AbstractProjectile) projectile).getHandle()).shootFromRotation(((CraftEntity) entity).getHandle(), entity.getLocation().getPitch(), entity.getLocation().getYaw(), roll, force, divergence);
    }
    ...
    public void damageItem(ItemStack item, int amount, Player player) {
        if (item.getItemMeta() instanceof Damageable damageable) {
            ...
            if (damageable.getDamage() > item.getType().getMaxDurability()) item.setType(Material.AIR);
